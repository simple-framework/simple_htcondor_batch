...
def get_supported_vos_with_quotas(augmented_site_level_config):
    supported_vos = augmented_site_level_config['supported_virtual_organizations']
    zero_quota_vo_count = 0
    total_defined_quota = 0
    total_quota = 100
    for vo in supported_vos:
        if "quota" not in vo:
            vo['quota'] = 0
            zero_quota_vo_count = zero_quota_vo_count+1
            continue
        elif "quota" in vo and vo['quota'] == 0:
            zero_quota_vo_count = zero_quota_vo_count + 1
            continue
        else:
            total_defined_quota += vo['quota']
    total_undefined_quota = total_quota - total_defined_quota
    if total_undefined_quota <= 0:
        return supported_vos
    per_undefined_vo_quota = total_undefined_quota/zero_quota_vo_count
    for vo in supported_vos:
        if vo['quota'] == 0:
            vo['quota'] = per_undefined_vo_quota
    return supported_vos


def get_quotas_for_fqans(fqans):
    zero_quota_fqan_count = 0
    total_defined_quota = 0
    total_quota = 100
    for fqan in fqans:
        if "quota" not in fqan:
            fqan['quota'] = 0
            zero_quota_fqan_count = zero_quota_fqan_count + 1
            continue
        elif "quota" in fqan and fqan["quota"] == 0:
            zero_quota_fqan_count = zero_quota_fqan_count + 1
            continue
        else:
            total_defined_quota += fqan["quota"]
    total_undefined_quota = total_quota - total_defined_quota
    if total_undefined_quota <= 0:
        return fqans
    per_undefined_fqan_quota = total_undefined_quota/zero_quota_fqan_count
    for fqan in fqans:
        if fqan["quota"] == 0:
            fqan["quota"] = per_undefined_fqan_quota
    return fqans
